...
func TestHandleLogout_RebuildsPostLogoutRedirectURIFromTrustedOrigin(t *testing.T) {
	...
	oidc := &TraefikOidc{
		...
		sessionManager: sm,
		// postLogoutRedirectURI left at its zero value, matching the
		// documented default ('/') described in docs/CONFIGURATION.md and
		// main.go.
	...
	if got != want {
		t.Fatalf("post_logout_redirect_uri = %q, want %q (derived from the persisted redirect_url origin)", got, want)
	}
}
