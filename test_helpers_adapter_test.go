...
func setupTestOIDCMiddleware(t *testing.T, config *Config) (*TraefikOidc, *httptest.Server) {
	...
	sessionManager, _ := NewSessionManager(config.SessionEncryptionKey, false, "", logger, "test-instance")
...
func createTestSession() *SessionData {
	...
	sessionManager, _ := NewSessionManager("test-encryption-key-32-characters", false, "", logger, "test-instance")
