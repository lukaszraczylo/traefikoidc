...
| Opaque token `iat` requirement | The introspection response for an opaque token must carry `iat`. A response with no `iat` is rejected once `maxTokenAgeSeconds` is set (it always is; see the config-reference row below). | Mirrors `enforceIatAge` on the JWT path: with an age bound configured, a token with nothing to bound its age against must fail closed, not skip the check. |
